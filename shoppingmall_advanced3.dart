...
class ShoppingMall {
  ...
  void _addProduct() {
    ...
    if (price != null && price > 0) {
      ...
      if (confirm == 'yes') {
        ...
        for (var product in productList) {
          int? productNum =
              int.tryParse(product.productCode.substring(1, 2) ?? '');
  ...
  void _removeProduct() {
    ...
    print(
        '\'${productToRemove.name}\' 상품 \'${productToRemove.price}\'원을 목록에서 제거하시겠습니까? 제거하시려면 Yes, 취소하시려면 No를 입력해주세요.');
...
// 필수기능: 장바구니에 담는 메서드 + 자유 도전기능: 최대 구매 수량 제한
  void addToCart(String input, int quantity) {
    ...
    print('장바구니에 \'${product.name}\' 상품이 \'$quantity개\' 담겼어요!');
  }
...
// findProduct 오류 해결하기 위한 메서드 추가
  ...
  void modifyCartQuantity(String input, int newQuantity) {
    ...
    if (newQuantity == 0) {
      ...
      cart[product] = newQuantity;
      print('${product.name} 상품의 수량이 $newQuantity개로 변경되었습니다.');
    }
  }
  ...
  void showTotal() {
    ...
    cart.forEach((product, quantity) {
      String key = '${product.gender} ${product.name} ${product.size}';
      itemCounts[key] = (itemCounts[key] ?? 0) + quantity;
      itemPrices[key] = product.price;
    });

    int totalItems = itemCounts.values.reduce((a, b) => a + b);

    int totalPrice = itemCounts.entries
        .map((entry) => entry.value * itemPrices[entry.key]!)
        .reduce((a, b) => a + b);

    List<String> cartDetails = [];

    itemCounts.forEach((key, value) {
      cartDetails.add('$key ${value}개 ${itemPrices[key]! * value}원');
    });

    print(
        '${currentUser.id}님 장바구니에 총 $totalItems개 상품이 담겨있습니다. ${cartDetails.join(", ")} 총 ${totalPrice}원 입니다!');
  }
